...
chan = comm.RicianChannel("SampleRate", 1/Ts, "MaximumDopplerShift", Fd, "KFactor", KFactor, "PathDelays", Tau, "AveragePathGains", PdB );
% chanraylei = rayleighchan(Ts, Fd, Tau, PdB);
chanraylei =comm.RayleighChannel("SampleRate", 1/Ts, "MaximumDopplerShift", Fd, "PathDelays", Tau, "AveragePathGains", PdB);
...
%瑞利信道&莱斯信道&natagami-m信道
        if (si==1)
            chan.release();
            chanraylei.release();
            chan.AveragePathGains = chan.AveragePathGains + snr(si);
            chanraylei.AveragePathGains = chanraylei.AveragePathGains + snr(si);
        else
            chan.release();
            chanraylei.release();
            chan.AveragePathGains = chan.AveragePathGains + snr(si) - snr(si-1);
            chanraylei.AveragePathGains = chanraylei.AveragePathGains + snr(si) - snr(si-1);
        end   
        % 调整这里的信噪比的单位关系
        yrician = awgn(chan(tx_sig_real.'),snr(si),'measured').';
        yraylei = awgn(chanraylei(tx_sig_real.'),snr(si),'measured').';
